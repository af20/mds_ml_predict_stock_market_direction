...
class Ticker():
  ...
  def _split_data(self):
      ...
      self.X_train_full, self.X_test, self.y_train_full, self.y_test = train_test_split(self.X, self.y, train_size=self.TRAIN_SIZE, random_state=42, shuffle=False) # train_size=int(self.TRAIN_SIZE*len(self.X))    |    2 * len(self.X) // 3)
      self.X_train, self.X_valid, self.y_train, self.y_valid = train_test_split(self.X_train_full, self.y_train_full, train_size=self.TRAIN_SIZE_on_VALIDATION, random_state=42, shuffle=False)
